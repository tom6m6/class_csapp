...
int isLessOrEqual(int x, int y) {
	...
	int ysign= (y >> 31 ) & ( 0x1 );
	int equ=!(x ^ y);
	return equ |(~(xsign ^ ysign))&(((x+ (~y)+1) >>31  )&( 0x1 ))|((xsign ^ ysign )&xsign); 
  	/*int xsign= (x >> 31 ) & ( 0x1 );
	int ysign= (y >> 31 ) & ( 0x1 );
	int equ=!(x ^ y);
	int con1= equ &  (0x1 & ((x + ~y + 1) >> 31));
...
unsigned floatScale2(unsigned uf) {
	...
	if(exp==0)return (uf & 0x80000000 )+(exp << 23)+(frac << 1);
...
int floatFloat2Int(unsigned uf) {
	...
	if(exp>=(bias+31))return 0x80000000;
	int v=1 << (exp -bias);
	int s= (uf >> 31) & 0x1;
       if(s)return -v;
